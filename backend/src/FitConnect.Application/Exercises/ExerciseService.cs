...
public class ExerciseService
{
    ...
    public async Task<Exercise> CreateAsync(Guid trainerId, string name, int defaultReps, int defaultSets, CancellationToken cancellationToken = default)
    {
        ...
        var exercise = new Exercise(Guid.NewGuid(), trainerId, name, defaultReps, defaultSets, demoVideoUrl: null);
        return await exerciseRepository.CreateAsync(exercise, cancellationToken);
    }

    public async Task UpdateAsync(Guid id, string name, int defaultReps, int defaultSets, CancellationToken cancellationToken = default)
    {
        var exercise = await exerciseRepository.GetByIdAsync(id, cancellationToken)
            ?? throw new ExerciseNotFoundException(id);

        exercise.UpdateDetails(name, defaultReps, defaultSets);
